/**
 * Sequence Node Logic Adapter
 *
 * 简化版：直接实现核心逻辑，不需要二次分发
 *
 * 设计说明：
 * - Sequence 节点的逻辑是固定的，不需要像 Action 节点那样进行二次分发
 ...
 * Sequence 节点逻辑：
 * - 按顺序执行子节点
 * - 任何子节点失败则整个 Sequence 失败
 * - 所有子节点成功则 Sequence 成功
 */
export class SequenceNodeLogicAdapter<
  TNodeType = string,
