import { ArrayExt } from "../Extension/ArrayExt";
import { MapExt } from "../Extension/MapExt";
import { SetExt } from "../Extension/SetExt";
import type { SingleEntryGraphNode } from "./SingleEntryGraphNode";
...
export class SingleEntryGraph<M extends SingleEntryGraphNode<T>, T> {
  ...
  // 自定义广度优先搜索，从某个节点开始，遇到节点后执行自定义的handler
  // onVisitNode的输入是一个node, 输出是boolean, 表示是否要遍历子节点
  public CustomBfsFromStartNodeId(fromId: T, onVisitNode) {
    let queue: T[] = [];
    queue.push(fromId);
    ...
    while (queue.length > 0) {
      ...
      for (let i = 0; i < levelSize; i++) {
        ...
        let node = this.GetVertexDetail(vertexId);
        let shoudVisitNext = onVisitNode(node);
        if (shoudVisitNext) {
  ...
  // 向下找
  // 广度优先搜索，从某个中间节点开始，向后遍历这个图，找到相连的节点
  public QueryReachableVertexIdsFromStartNodeId(fromId: T, includeSelf: Boolean) : Set<T> {
  ...
  public GetVertexIdsToRoot(vertexId, includeSelf) : T[] {
    let vertexIdsToRootPath : T[] = [];
    if (includeSelf) {
      vertexIdsToRootPath.push(vertexId);
    }
    let iterNode = this.GetVertexDetail(vertexId);
    while (iterNode != null) {
      let prevNodeIds = Array.from(this.GetPrevVertexIds(iterNode.GetVertexId()));
      // 树中的parent只有一个
      if (prevNodeIds.length > 0) {
        let parentNodeId = prevNodeIds[0];
        iterNode = this.GetVertexDetail(parentNodeId);
        vertexIdsToRootPath.push(iterNode.GetVertexId());
      } else {
        iterNode = null;
      }
    }
    return vertexIdsToRootPath;
  }
